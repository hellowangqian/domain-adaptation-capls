...
for source_domain_index = 1:length(domains)
    ...
    for target_domain_index = 1:length(domains)
        ...
        %my_tsne(domainS_features(domainS_labels<classTh,:),domainT_features(domainT_labels<classTh,:),domainS_labels(domainS_labels<classTh),domainT_labels(domainT_labels<classTh),classTh);
...
save(['officeHome-SP-' num2str(using_sp) '-PcaDim-' num2str(opts.ReducedDim) '-LppDim-' num2str(d) '-T-' num2str(T) '.mat'],'all_acc_per_class','all_acc_per_image','mean_acc_per_class','mean_acc_per_image');
